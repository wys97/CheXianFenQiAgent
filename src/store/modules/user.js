import { loginByUsername, logout, updatePassword, getUserInfo, updatePasswordAgain } from '@/api/user'
...
const user = {
    ...
    actions: {
        ...
        // 修改密码
        updatePasswordAgain({ state }, passwordInfo) {
            const newPassword = passwordInfo.newPassword
            const oldPassword = passwordInfo.oldPassword
            const confirmPassword = passwordInfo.confirmPassword
            return new Promise((resolve, reject) => {
                updatePasswordAgain({ newPassword, oldPassword, confirmPassword }).then(response => {
                    if (response.data.code === '200') {
                        removeIsLogin()
                    }
                    resolve(response)
