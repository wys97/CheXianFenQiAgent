      ...
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="审核状态" class="query-select" prop="recheckStatus">
          <el-select v-model="listQuery.recheckStatus" style="width:150px;">
            ...
            <el-option v-if="finalfirstStatusList" v-for="(val,key) in finalfirstStatusList" :label="val" :value='key' :key='key'>
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            ...
            <router-link :to="'finalfirstDetail/' + scope.row.supplierId" class="router-link">审核详情</router-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { getListData,getRegisterStatusList,finalfirstStatusList,finallyStatusList } from '@/api/approveManage/finalfirstList'
  ...
  export default {
    ...
    data () {
      return {
        ...
        finalfirstStatusList:'',//列表查询-审核状态下拉
    ...
    methods: {
      ...
      getRecheckStatusList (){//列表查询-审核状态下拉
      	finalfirstStatusList().then(res => {
          if (res.data.code === '200') {
          	this.finalfirstStatusList= res.data.data
          } else {
            this.$message.error(res.data.message)
          }
        })
