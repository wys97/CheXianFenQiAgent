  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
      ...
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="审核状态" class="query-select" prop="recheckStatus">
          <el-select v-model="listQuery.recheckStatus" style="width:150px;">
            ...
            <el-option v-if="recheckStatusList" v-for="(val,key) in recheckStatusList" :label="val" :value='key' :key='key'>
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        ...
        label="复审额度（万元）">
        <template slot-scope="scope" >
          <span >{{scope.row.creditLineAuditor2}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            ...
            <router-link :to="'recheckDetail/' + scope.row.supplierId" class="router-link">审核详情</router-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { getListData,getRegisterStatusList,recheckStatusList,finallyStatusList } from '@/api/approveManage/recheckList'
  ...
  export default {
    ...
    methods: {
      ...
      getRecheckStatusList (){//列表查询-审核状态下拉
      	recheckStatusList().then(res => {
          if (res.data.code === '200') {
          	this.recheckStatusList= res.data.data
          } else {
            this.$message.error(res.data.message)
          }
        })
