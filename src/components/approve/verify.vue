<template>
	<div>
			    ...
			    <el-form  label-width="170px" size="mini" label-position="right">
			    	...
			    	<div class="info-list">
			    		...
			    		<el-form-item label="姓名：">
							...
							<el-input  class="must_input"  v-else v-model="controllerData.name"  style="width: 300px;">
	                		</el-input>
						</el-form-item>
						<el-row>
						  	<el-col :span="12">
							  	<el-form-item label="身份证号：">
									...
									<el-input v-else  class="must_input" v-model="controllerData.idcardNo"  style="width: 300px;">
			                		</el-input>
								</el-form-item>
						  	</el-col>
						  	<el-col :span="12">
			    		...
			    		<el-form-item label="地址：">
							...
							<el-input v-else v-model="controllerData.address"  class="must_input"  style="width: 300px;">
	                		</el-input>
						</el-form-item>
			    		<el-form-item label="手机号：">
							<span v-if="readOnly" class="input_read">{{controllerData.mobile}}</span>
							<el-input v-else v-model="controllerData.mobile"   class="must_input" style="width: 300px;">
	                		</el-input>
						</el-form-item>
			    		...
			    		<el-form-item label="身份证：">
	                		<template>
	                          <Upload
	                          	...
	                          	actionUrl="/admin/supply_data/upload_image_file"></Upload>
	                        </template>
	                        <template>
	                          <Upload
			    		...
			    		<el-form-item label="从业年数：">
							...
							<el-input v-else v-model="controllerData.workingYears"  class="must_input"  style="width: 300px;">
								<template slot="append">年</template>
	                		</el-input>
						</el-form-item>
			    		<el-form-item label="总资产：">
							<span v-if="readOnly" class="input_read">{{controllerData.totalAssets}} 元</span>
							<el-input v-else  class="must_input" v-model="controllerData.totalAssets"  style="width: 300px;">
		...
		<!--企业系统销售数据-->
		<el-collapse @change="changeVerifyEnterpriseSale1" v-model="activeVerifyEnterpriseSaleNames1" class="marginBotten">
		...
		<!--企业销售情况-->
		<el-collapse @change="changeVerifyEnterpriseSale2" v-model="activeVerifyEnterpriseSaleNames2" class="marginBotten">
			    	...
			    	<div class="info-list">
			    		...
			    		<div class="sale_box">
			    			<div v-for="(item,index) in bankData" class="sale_box">
				    			...
				    			<el-form-item label="月份：" v-if="bankTimeList">
									<template>
								    	<el-date-picker
								    		:disabled="readOnly"
			    		...
			    		<el-input v-else type="textarea" :rows="4" :disabled="readOnly" v-model="verifyInvoiceBankSaleData.saleAnalysis">
	                	</el-input>
						...
						<el-input v-else class="must_input" v-model="verifyInvoiceBankSaleData.saleEstimatedAmount" style="width: 300px;">
			    			<template slot="append">万元</template>
	                	</el-input>
			    		<div class="lh_title">销售稳定性（银行流水、纳税销售数据、平台 交易记录或生产能耗(电费等)较上年）</div>
			    		<span v-if="readOnly" class="input_read">{{verifyInvoiceBankSaleData.saleGrowth?verifyInvoiceBankSaleData.saleGrowth:'-'}} %</span>
						<el-input v-else class="must_input" v-model="verifyInvoiceBankSaleData.saleGrowth" style="width: 300px;">
			    			<template slot="append">%</template>
	                	</el-input>
			    		<div class="lh_title">销售额稳定程度</div>
	                	<el-form-item label="销售额稳定程度：" style="margin-top: 15px;">
	                		<template>
	                			<span v-if="readOnly" class="input_read">{{verifyInvoiceBankSaleData.saleStableText?verifyInvoiceBankSaleData.saleStableText:'-'}}</span>
								<el-radio-group v-else class="must_input" v-model="verifyInvoiceBankSaleData.saleStable">
			    	...
			    	<div class="info-list">
			    		...
			    		<el-form-item label="是否出现回收困难：">
	                		<template>
							    ...
							    <el-radio-group v-else class="must_input" v-model="verifyBusinessOperatCapitalData.businessReceivableHard">
									<el-radio label="1">是</el-radio>
									<el-radio label="0">否</el-radio>
								</el-radio-group>
							</template>
						...
						<div class="table_top" style="text-align: center;line-height: 40px;margin-bottom: 20px;">
							...
							<el-row style="margin-bottom: 0;">
								<el-col :span="4" class="table_td" style="line-height: 40px;">
							      	...
							      	<el-input v-else  class="must_input" v-model="verifyBusinessOperatCapitalData.turnoverReceivable">
	                				</el-input>
								</el-col>
								<el-col :span="4" class="table_td" style="line-height: 40px;">
							      	...
							      	<el-input v-else class="must_input"  v-model="verifyBusinessOperatCapitalData.turnoverStock">
	                				</el-input>
								</el-col>
								<el-col :span="4" class="table_td" style="line-height: 40px;">
							      	...
							      	<el-input v-else  class="must_input" v-model="verifyBusinessOperatCapitalData.turnoverPayable">
	                				</el-input>
								</el-col>
								<el-col :span="4" class="table_td" style="line-height: 40px;">
							      	...
							      	<el-input v-else class="must_input"  v-model="verifyBusinessOperatCapitalData.turnoverPaid">
	                				</el-input>
								</el-col>
								<el-col :span="4" class="table_td" style="line-height: 40px;">
							      	...
							      	<el-input v-else  class="must_input" v-model="verifyBusinessOperatCapitalData.turnoverReceived">
	                				</el-input>
								</el-col>
								<el-col :span="4" class="table_td" style="line-height: 40px;">
							      	...
							      	<el-input v-else  class="must_input" v-model="verifyBusinessOperatCapitalData.turnoverActualCapital">
	                				</el-input>
								</el-col>
							</el-row>
						</div>
						<div class="lh_title">依据主要产品的售价、成本、费用，与管理者沟通讨论，估算存纯利润率</div>
				      	...
				      	<el-input  v-else  class="must_input" v-model="verifyBusinessOperatCapitalData.profitRate" style="width: 300px;">
	                		<template slot="append">%</template>
						</el-input>

	                	<el-form-item label="最近一年盈利：" style="margin-top: 15px;">
	                		<template>
	                			<span v-if="readOnly" class="input_read">{{verifyInvoiceBankSaleData.recentYearProfit?'是':'否'}}</span>
								<el-radio-group v-else  class="must_input" v-model="verifyBusinessOperatCapitalData.recentYearProfit">
			    	...
			    	<div class="info-list" v-if="verifyEnterpriseDebtData">
			    		...
			    		<el-form-item label="存在金融机构贷款：">
							<template>
							    ...
							    <el-radio-group v-else class="must_input" v-model="verifyEnterpriseDebtData.finance.has">
								    ...
								    <el-table-column
								      ...
								      label="借款用途">
								      	<template slot-scope="scope" v-if="borrowPurposeList">
								        	<span v-if="readOnly" class="table_input_read">{{scope.row.borrowPurposeText?scope.row.borrowPurposeText:'-'}}</span>
								      		<el-select v-else v-model="scope.row.borrowPurpose"  placeholder="请选择">
											    <el-option v-for="(val,key) in borrowPurposeList" :key="key" :label="val" :value="key"></el-option>
											</el-select>
								       	</template>
								    </el-table-column>
								    <el-table-column
			    		...
			    		<el-form-item label="存在非金融机构贷款：">
							<template>
							    ...
							    <el-radio-group v-else class="must_input" v-model="verifyEnterpriseDebtData.nonfinance.has">
									<el-radio :label="true">有</el-radio>
									<el-radio :label="false">无</el-radio>
								</el-radio-group>
							</template>
			    		...
			    		<el-form-item label="是否存在：">
							<template>
							    ...
							    <el-radio-group v-else class="must_input" v-model="verifyEnterpriseDebtData.borrow.has">
									<el-radio :label="true">有</el-radio>
									<el-radio :label="false">无</el-radio>
								</el-radio-group>
							</template>
			    		...
			    		<el-form-item label="是否存在：">
							<template>
							    ...
							    <el-radio-group v-else class="must_input" v-model="verifyEnterpriseDebtData.guarantee.has">
									<el-radio :label="true">有</el-radio>
									<el-radio :label="false">无</el-radio>
								</el-radio-group>
							</template>
		...
		<!--信用情况-->
		<el-collapse @change="changeVerifyCredit" v-model="activeVerifyCreditNames" class="marginBotten">
			    ...
			    <el-form  label-width="170px" size="mini" label-position="right">
					<div class="info-list" v-if="verifyCreditSituationData">
			    		<el-form-item label="历史征信记录：" style="margin-bottom: 18px !important;">
							<template>
								<span v-if="readOnly" class="input_read">{{verifyCreditSituationData.creditScoreText}}</span>
							    <el-radio-group v-else class="must_input" v-model="verifyCreditSituationData.creditScore">
			    		...
			    		<el-form-item label="征信查询记录：" style="margin-bottom: 18px !important;">
							<template>
								<span v-if="readOnly" class="input_read">{{verifyCreditSituationData.creditQueryTimesText}}</span>
							    <el-radio-group v-else  class="must_input" v-model="verifyCreditSituationData.creditQueryTimes">
			    		...
			    		<el-form-item label="互联网信息核查：" style="margin-bottom: 18px !important;">
							<template>
								<span v-if="readOnly" class="input_read">{{verifyCreditSituationData.internetOpinionText}}</span>
							    <el-radio-group v-else  class="must_input" v-model="verifyCreditSituationData.internetOpinion">
			    		...
			    		<el-form-item label="法律诉讼核查：">
							<template>
								<span v-if="readOnly" class="input_read">{{verifyCreditSituationData.litigationSeverityText}}</span>
							    <el-radio-group v-else  class="must_input" v-model="verifyCreditSituationData.litigationSeverity">
						...
						<div v-if="!readOnly" style="text-align: right;">
							<el-button  @click="doSaveCreditSituation" v-if="!roleHasPermissions['supplier_manage_maintain_list_save']"  type="primary" size="mini">保存</el-button>
		                </div>
					</div>
			    </el-form>
			</el-collapse-item>
		...
		<!--担保情况-->
		<el-collapse @change="changeVerifyGuarantee" v-model="activeVerifyGuaranteeNames" class="marginBotten">
			    ...
			    <el-form  label-width="170px" size="mini" label-position="right">
					<div class="info-list" v-if="verifyGuaranteeValueData">
			    		<el-form-item label="担保价值认可度：">
							<template>
								<span v-if="readOnly" class="input_read">{{verifyGuaranteeValueData.guaranteeValueText}}</span>
							    <el-radio-group v-else class="must_input" v-model="verifyGuaranteeValueData.guaranteeValue">
						...
						<div v-if="!readOnly" style="text-align: right;">
							<el-button  @click="doSaveGuaranteeValue" v-if="!roleHasPermissions['supplier_manage_maintain_list_save']"  type="primary" size="mini">保存</el-button>
		                </div>
					</div>
			    </el-form>
			</el-collapse-item>
			    		...
			    		<el-form-item v-if="verifySpotCheckData.spotCheck=='1'" label="尽调原始证据：">
							<template v-if="verifySpotCheckData.spotCheckFiles" v-for="item in verifySpotCheckData.spotCheckFiles">
								<Uploads
	                          	...
	                          	:fileUrl="item.url"
	                          	:dataObj="uploadImgData"
	                          	actionUrl="/admin/check/upload_image"></Uploads>
							</template>
							<div v-if="!readOnly">
			    ...
			    <el-form  label-width="170px" size="mini" label-position="right">
					<div class="info-list" v-if="verifyCostOtherData">
			    		<el-form-item label="其他（满足其中一条）：">
							<template>
								<span v-if="readOnly" class="input_read">
									{{verifyCostOtherData.costOther===true?'是':verifyCostOtherData.costOther===false?'否':''}}
								</span>
						...
						<div v-if="!readOnly" style="text-align: right;">
							<el-button  @click="doSaveCostOther" v-if="!roleHasPermissions['supplier_manage_maintain_list_save']"  type="primary" size="mini">保存</el-button>
		                </div>
					</div>
			    </el-form>
			</el-collapse-item>
...
<script>
	...
	import { comboboxBankList, collateralType, collateralSource,maritalStatus,educationBackground,saleStable,creditScore,creditQueryTimes,internetOpinion,litigationSeverity,guaranteeValue} from '@/api/common'
	...
	export default {
			...
			saleStatOthersList(val,oldVal){
				...
				for(let i=0;i<this.saleStatOthersList.length;i++){
					_arr.push({
						...
						endTime:this.saleStatOthersList[i].data[this.saleStatOthersList[i].data.length-1].key,//结束时间
					})
				}
		...
		methods: {
		    ...
		    changeVerifyEnterpriseSale1 (val){//展开企业系统销售数据
		      	if(val.length===0){return}
		      	this.getQueryVerifyEnterpriseSale();//尽调检查-企业销售情况-查询
		      	this.getThirdPartyPlatformTypeList();//第三方频台下拉
		      	this.getBankStatTypeList();//银行流水验证下拉
		    },
		    changeVerifyEnterpriseSale2 (val){//展开企业销售情况
		      	if(val.length===0){return}
//		      	this.getQueryVerifyEnterpriseSale();//尽调检查-企业销售情况-查询
		      	this.getQueryVerifyInvoiceBankSale();//尽调检查-发票银行流水-查询
						this.getSaleStableList();//销售额稳定程度下拉
		    ...
		    changeVerifyCredit  (val){//展开信用情况
		      	if(val.length===0){return}
		      	this.getQueryVerifyCreditSituation();//尽调检查-信用情况-查询
		    },
		    changeVerifyGuarantee (val){//展开担保信息
		      	if(val.length===0){return}
		      	this.getQueryVerifyGuaranteeValue();//尽调检查-担保信息-查询
		    },
		    changeVerifyOther (val){//展开其他信息
		      	if(val.length===0){return}
		      	this.getQueryVerifyCostOther();//尽调检查-其他信息-查询
		    },
			...
			doSaveController(){//尽调检查-实际控制人-保存
				if(!this.controllerData.name){this.$message.error('姓名不能为空！');return;}
				if(!this.controllerData.idcardNo){this.$message.error('身份证号不能为空！');return;}
				if(!this.controllerData.gender){this.$message.error('性别不能为空！');return;}
				if(!this.controllerData.address){this.$message.error('地址不能为空！');return;}
				if(!this.controllerData.mobile){this.$message.error('手机号不能为空！');return;}
				if(!this.controllerData.workingYears){this.$message.error('从业年数不能为空！');return;}
				if(!this.controllerData.totalAssets){this.$message.error('总资产不能为空！');return;}
			...
			changeSaleStatSelfTime(){//自有系统平台销售数据 -------------改变时间
				if(!this.saleStatSelfStartTime||!this.saleStatSelfEndTime){//非空判断
					return
				}
				if(this.saleStatSelfStartTime&&(typeof this.saleStatSelfStartTime)==='object'){
					this.saleStatSelfStartTime = Format(this.saleStatSelfStartTime, 'yyyy-MM');
				}
				if(this.saleStatSelfEndTime&&(typeof this.saleStatSelfEndTime)==='object'){
				...
				for(let i=0;i<_length;i++){
					...
					for(let j=0;j<this.saleStatSelfList.data.length;j++){
						if(this.saleStatSelfList.data[j].key ===Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM')){
							_item = {
								key:Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM'),
			...
			doUploadTitan(){//加载
				...
				if(!this.saleStatTitanStartTime||!this.saleStatTitanEndTime){//非空判断
					this.$message.error('时间不能为空！');return;
				}
				if(this.saleStatTitanStartTime&&(typeof this.saleStatTitanStartTime)==='object'){
					this.saleStatTitanStartTime = Format(this.saleStatTitanStartTime, 'yyyy-MM');
				}
				if(this.saleStatTitanEndTime&&(typeof this.saleStatTitanEndTime)==='object'){
					this.saleStatTitanEndTime = Format(this.saleStatTitanEndTime, 'yyyy-MM');
				}
				let _data = {
				...
				this.Api.titanDataByDate(_data).then(res => {
					if(res.data.code === '200') {
						this.saleStatTitanList.data = res.data.data;
						this.saleStatTitanStartTime  = res.data.data[0].key;
						this.saleStatTitanEndTime  = res.data.data[res.data.data.length-1].key;
...
//						this.saleStatOthersList = res.data.data.saleStatOthers;
			...
			changeStatTitanTime(){//泰坦云数据 -------------改变时间
				return;//先注释不用
				if(!this.saleStatTitanStartTime  ||!this.saleStatTitanEndTime){//非空判断
					return
				}
				...
				for(let i=0;i<_length;i++){
					...
					for(let j=0;j<this.saleStatTitanList.data.length;j++){
						if(this.saleStatTitanList.data[j].key ===Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM')){
							_item = {
								key:Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM'),
			...
			changeSaleStatOthersTime(index){//其他平台销售数据 -------------改变时间
				if(!this.saleStatOthersTimeList[index].startTime||!this.saleStatOthersTimeList[index].endTime){//非空判断
					return
				}
				if(this.saleStatOthersTimeList[index].startTime&&(typeof this.saleStatOthersTimeList[index].startTime)==='object'){
					this.saleStatOthersTimeList[index].startTime = Format(this.saleStatOthersTimeList[index].startTime, 'yyyy-MM');
				}
				if(this.saleStatOthersTimeList[index].endTime&&(typeof this.saleStatOthersTimeList[index].endTime)==='object'){
					this.saleStatOthersTimeList[index].endTime = Format(this.saleStatOthersTimeList[index].endTime, 'yyyy-MM');
				}
				let startTimeNum = new Date(this.saleStatOthersTimeList[index].startTime).getTime();
				let endTimeNum = new Date(this.saleStatOthersTimeList[index].endTime).getTime();
				if(startTimeNum>endTimeNum){
					this.$message.error('开始月份不能大于结束月份')
					return
				}
				...
				for(let i=0;i<_length;i++){
					...
					for(let j=0;j<this.saleStatOthersList[index].data.length;j++){
						if(this.saleStatOthersList[index].data[j].key ===Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM')){
							_item = {
								key:Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM'),
			...
			changeInvoiceTime(){//发票数据 -------------改变时间
				if(!this.invoiceStartTime||!this.invoiceEndTime){//非空判断
					return
				}
				...
				for(let i=0;i<_length;i++){
					...
					for(let j=0;j<this.invoiceData.data.length;j++){
						if(this.invoiceData.data[j].key ===Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM')){
							_item = {
								key:Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM'),
			...
			changeBankTime(index){//银行数据 -------------改变时间
				if(!this.bankTimeList[index].startTime||!this.bankTimeList[index].endTime){//非空判断
					return
				}
				if(this.bankTimeList[index].startTime&&(typeof this.bankTimeList[index].startTime)==='object'){
					this.bankTimeList[index].startTime = Format(this.bankTimeList[index].startTime, 'yyyy-MM');
				}
				if(this.bankTimeList[index].endTime&&(typeof this.bankTimeList[index].endTime)==='object'){
					this.bankTimeList[index].endTime = Format(this.bankTimeList[index].endTime, 'yyyy-MM');
				}
				...
				for(let i=0;i<_length;i++){
					...
					for(let j=0;j<this.bankData[index].data.length;j++){
						if(this.bankData[index].data[j].key ===Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM')){
							_item = {
								key:Format(new Date(startTimeNum+2678400000*i), 'yyyy-MM'),
			...
			doSaveBankData(){//尽调检查-发票银行流水-保存
				if(!this.verifyInvoiceBankSaleData.saleEstimatedAmount){this.$message.error('可验证销售总额测算不能为空！');return;}
				if(!this.verifyInvoiceBankSaleData.saleGrowth){this.$message.error('销售稳定性不能为空！');return;}
				if(!this.verifyInvoiceBankSaleData.saleStable){this.$message.error('销售额稳定程度不能为空！');return;}
				let data = {};
			...
			doSaveBusinessOperatCapital(){//尽调检查-企业主营业务营运资金循环-保存
				if(this.verifyBusinessOperatCapitalData.businessReceivableHard==null||this.verifyBusinessOperatCapitalData.businessReceivableHard==''){
					this.$message.error('是否出现回收困难不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.turnoverReceivable){
					this.$message.error('应收账款周转不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.turnoverStock){
					this.$message.error('存货周转不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.turnoverPayable){
					this.$message.error('应付账款周转不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.turnoverPaid){
					this.$message.error('预付账款周转不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.turnoverReceived){
					this.$message.error('预收账款周转不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.turnoverActualCapital){
					this.$message.error('实际营运资金周转不能为空！');return;
				}
				if(!this.verifyBusinessOperatCapitalData.profitRate){
					this.$message.error('估算存纯利润率不能为空！');return;
				}
				if(this.verifyBusinessOperatCapitalData.recentYearProfit==null){
					this.$message.error('最近一年盈利不能为空！');return;
				}
				let data = {};
				data.supplierId = this.supplierId
			...
			doSaveEnterpriseFinance(){//尽调检查-企业财务统计-保存
				if(!this.verifyEnterpriseFinanceData.equityAmount){
					this.$message.error('总有形资产不能为空！');return;
				}
				if(!this.verifyEnterpriseFinanceData.liabilityAmount){
					this.$message.error('总负债不能为空！');return;
				}
				let data = {};
				data.supplierId = this.supplierId
			...
			getQueryVerifyCreditSituation (){//尽调检查-信用情况-查询
				this.Api.creditSituation(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			getQueryVerifyGuaranteeValue (){//尽调检查-担保信息-查询
				this.Api.guaranteeValue(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			doSaveGuaranteeValue(){//尽调检查-担保信息-保存
				if(!this.verifyGuaranteeValueData.guaranteeValue){
					this.$message.error('担保价值认可度不能为空！');return;
				}
				let data = baseUtil.copy(this.verifyGuaranteeValueData)
		      	data.supplierId = this.supplierId
		      	this.Api.saveGuaranteeValue(data).then(res => {
		          if (res.data.code === '200') {
		            this.$message.success(res.data.message)
		          } else {
		            this.$message.error(res.data.message)
			...
			getQueryVerifyCostOther (){//尽调检查-其他信息-查询
				this.Api.costOther(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			doSaveCostOther(){//尽调检查-其他信息-保存
				let data = baseUtil.copy(this.verifyCostOtherData)
		      	data.supplierId = this.supplierId
		      	this.Api.saveCostOther(data).then(res => {
		          if (res.data.code === '200') {
		            this.$message.success(res.data.message)
		          } else {
		            this.$message.error(res.data.message)
			...
			getBorrowPurposeList(){//借款用途-下拉列表
				this.Api.borrowPurpose().then(res => {
					if(res.data.code === '200') {
			...
			getCreditScoreList(){//历史征信资料---单选
				creditScore().then(res => {
					if(res.data.code === '200') {
			...
			getCreditQueryTimesList(){//征信查询记录---单选
				creditQueryTimes().then(res => {
					if(res.data.code === '200') {
			...
			getInternetOpinionList(){//互联网信息核查---单选
				internetOpinion().then(res => {
					if(res.data.code === '200') {
			...
			getLitigationSeverityList(){//法律诉讼核查---单选
				litigationSeverity().then(res => {
					if(res.data.code === '200') {
			...
			getGuaranteeValueList(){//担保价值认可度---单选
				guaranteeValue().then(res => {
					if(res.data.code === '200') {
