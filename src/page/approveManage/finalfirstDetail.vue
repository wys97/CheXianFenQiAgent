		...
		<div class="top">
			...
			<div class="flex-item" style="text-align: right;padding-top: 0;">
				...
				<el-button 
					...
					checkHeadData.status=='REVIEW_SUCCESS'?'success':
				...
				<el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
					<el-tab-pane label="评级打分卡" name="first">
						<div class="tab-content">
							<template>
								<Scorecard :api-url="'finalfirstDetail'" :supplier-id="supplierId" :readOnly="true">
								</Scorecard>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane class="tab_content" label="尽调检查" name="second">
						<div class="tab-content">
							<template v-if="verifyShow">
								<Verify :api-url="'finalfirstDetail'" :supplier-id="supplierId" :readOnly="true">
								</Verify>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane label="初审结论" name="three">
						<div class="tab-content">
							<template v-if="checkData">
								<Check :api-url="'finalfirstDetail'" :supplier-id="supplierId" :readOnly="true" :check-data="checkData">
								</Check>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane label="复审结论" name="four">
						<div class="tab-content">
							<template v-if="recheckData">
								<Recheck :api-url="'finalfirstDetail'" :supplier-id="supplierId" :readOnly="true" :recheck-data="recheckData">
								</Recheck>
							</template>
						</div>
					</el-tab-pane>
					<el-tab-pane label="终审一审备注" name="five">
						<div class="tab-content">
							<div class="info-list">
								<el-form ref="verifyFirmInfoForm" label-width="170px" size="mini" label-position="right">
									<div class="lh_title">终审一审备注说明</div>
				...
				<ul>
					...
					<li v-if="checkHeadData.status==='PENDING'">
						<div class="fk_btn_box">
							<el-button @click="doPass" v-if="!roleHasPermissions['finance_manage_register_list_audit']" type="success">终审一审通过</el-button>
						</div>
					</li>
					<li v-if="checkHeadData.status==='PENDING'">
						<div class="fk_btn_box">
							<el-button @click="doReject " v-if="!roleHasPermissions['finance_manage_register_list_audit']" type="danger">终审一审拒绝</el-button>
						</div>
					</li>
					<!--<li>
						<div class="fk_btn_box">
	...
	export default {
		...
		methods: {
			...
			getQueryCheckHead() { //押品管理-信息登记-头部信息
				finalfirstDetail.checkHead(this.supplierId).then(res => {
			...
			getQuerySignConclusion() { //初审授信结论-查询
				finalfirstDetail.signConclusion(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			getQueryRecheckData() { //复审授信结论-查询
				finalfirstDetail.recheckDetail(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			//    	finalfirstDetail.scorecardQuery(this.supplierId).then(res => {
			//        if (res.data.code === '200') {
			...
			//      }).catch(() => {})
			//    },
			getQueryReviewDetail(){//终审一审备注查询
				finalfirstDetail.reviewDetail(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			doReviewSave(){//终审一审备注保存
				let data={
					remark:this.reviewRemark,
					supplierId:this.supplierId
				}
				finalfirstDetail.reviewSave(data).then(res => {
					if(res.data.code === '200') {
			...
			getSignEnterpriseStrengthList() { //企业实力和业绩单选数据
				finalfirstDetail.signEnterpriseStrength(this.supplierId).then(res => {
					if(res.data.code === '200') {
			...
			doPass() { //通过
				this.$confirm('确认通过终审一审？', '操作提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					finalfirstDetail.checkPass(this.supplierId).then(res => {
						if(res.data.code === '200') {
							this.$message.success(res.data.message)
							this.getQueryCheckHead(); //头部信息
						} else {
			...
			approveFinal() { //确认拒绝提交
				...
				finalfirstDetail.checkReject(data).then(res => {
					if(res.data.code === '200') {
						this.$message.success(res.data.message)
