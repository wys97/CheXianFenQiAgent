...
const app = {
  ...
  mutations: {
    ...
    ADD_VISITED_VIEWS: (state, view) => {
      ...
      if (view.name === '分期申请详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '分期申请详情' })
        if (findIndex === -1) {
      ...
      if (view.name === '新增分期申请') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '新增分期申请' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '资料详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '资料详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '初审详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '初审详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '复核拒绝的审批详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '复核拒绝的审批详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
      ...
      if (view.name === '终审一审详情') {
        const findIndex = state.visitedViews.findIndex((v) => { return v.name === '终审一审详情' })
        if (findIndex === -1) {
          state.visitedViews.push({ name: view.name, path: view.path, componentName: view.matched[view.matched.length - 1].components.default.name })
        } else {
          state.visitedViews[findIndex].path = view.path
