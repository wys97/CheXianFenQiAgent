  ...
  <div class="page-container" ref="page-container" id="page-container"><!---商户管理-商户入驻--->
    <div class="filter-container" ref="filter-container" id="filter-container">
      <el-form :inline="true" :model="listQuery" ref="listQueryForm">
        ...
        <el-form-item label="类型" class="query-select" prop="statusType">
          <el-select v-model="listQuery.statusType" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
            <el-option v-if="statusTypeList" v-for="(val,key) in statusTypeList" :label="val" :value='key' :key='key'>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="复核状态" class="query-select" prop="reviewStatus">
          <el-select v-model="listQuery.reviewStatus" style="width:150px;">
            <el-option label="全部" value="">
            </el-option>
            <el-option v-if="reviewStatusList" v-for="(val,key) in reviewStatusList" :label="val" :value='key' :key='key'>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="list"  @current-change="handleSelectionChange" highlight-current-row  :max-height=tableMaxHeight>
      ...
      <el-table-column
        ...
        label="审核状态">
        <template slot-scope="scope" >
          <span >{{scope.row.reviewStatusText}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope" v-if="!roleHasPermissions['supplier_manage_register_list_view']">
            ...
            <router-link :to="'reviewRecheckDetail/' + scope.row.supplierId" class="router-link">审核详情</router-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { getListData,getRegisterStatusList,statusType,reviewStatus } from '@/api/approveManage/reviewRecheckList'
  import { Format } from '@/utils/index'

  export default {
    name: 'reviewRecheckList',
    filters: {
    ...
    mounted () {
      ...
      this.getStatusTypeList ();//类型下拉
      this.getReviewStatusList ();//复核下拉
    ...
    methods: {
      ...
      getReviewStatusList (){///复核下拉
      	reviewStatus().then(res => {
          if (res.data.code === '200') {
          	this.reviewStatusList= res.data.data
          } else {
            this.$message.error(res.data.message)
          }
        })
