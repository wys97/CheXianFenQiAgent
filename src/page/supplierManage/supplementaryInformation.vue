            ...
            >
              ...
              >
                ...
                <el-form-item label="户籍城市：">
                  <el-select
                    v-model="basicInformation.city_code"
                    placeholder="请选择"
                    @change="selectCity"
                  >
                    <el-option
          ...
          <el-tab-pane label="车辆信息">
            ...
            <el-form label-width="180px" style="width:100%">
              <el-form-item label="证件类型：" prop="insuredPhone">
                <span v-if="this.carData.licenseType!=='QUALIFICATION'">行驶证</span>
                <span v-else>合格证</span>
              </el-form-item>
              <el-form-item v-if="this.carData.licenseType!=='QUALIFICATION'" label="车牌号：" prop="identityCardFile">
              ...
              <el-form-item label="行驶证：" v-if="this.carData.licenseType!=='QUALIFICATION'">
                <div style="display:flex; align-items:center">
          ...
          <el-tab-pane label="分期申请信息">
            ...
            >
              ...
              <el-form-item label="上传投保单：" prop="policyholderUrl">
                <el-upload
                  :limit="1"
                  action="/agent-api/apply_supplement/upload_insurance_policy"
              ...
              <el-form-item label="分期期数：" prop="installmentPeriods">
                <span>{{this.listData.productInstitution.periods}}</span>
              </el-form-item>
              ...
              <el-row>
                <el-col :span="20">
                  <el-form-item label="商业险金额：" prop="commercialInsurance">
                    <el-input v-model="listData.commercialInsurance"></el-input>元
                  </el-form-item>
                </el-col>
              ...
              <el-form-item label="交强险金额：" prop="compulsoryInsurance">
                <el-input v-model="listData.compulsoryInsurance"></el-input>元
              </el-form-item>
              <el-form-item label="车船税金额：" prop="motorAndShipInsurance">
                <el-input v-model="listData.motorAndShipInsurance"></el-input>元
              </el-form-item>
            </el-form>
            <hr width="90%" />
            <div v-if="trialDialogVisible" style="margin:40px 0 0 170px">
              ...
              >平台首付：{{this.trial.firstAmount}}元（交强{{this.trial.strongAmount}}元+车船{{this.trial.taxAmont}}元+保证金{{this.trial.depositAmount}}元+线上服务费{{this.trial.customerOnlineAmount}}元）</p>
              <p style="line-height:30px; padding-left:80px">首付保证金率=100%/总期数</p>

              <el-table
                :data="trial.installmentPlanList"
                style="width: 60%; margin-top:30px"
                border
              >
                <el-table-column prop="installmentNo" label="期数" width="180" align="center"></el-table-column>
                <el-table-column
                  prop="installmentCurrentAmount"
                  label="应还金额"
                  width="180"
                  align="center"
                ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getProvinceAndCity() {
      axiosRequest({
        url: "" + "/agent-api/order_supplement/area-info",
        method: "post",
        headers: {
    ...
    uploadPolicy(value) {
      ...
      if (
        ...
        reader.onload = e => {
          ...
          axiosRequest({
            url: "" + "/agent-api/apply_supplement/upload_insurance_policy",
            method: "post",
            data: { ...data },
    ...
    countBtn() {
      ...
      axiosRequest({
        url: "" + "/agent-api/order_supplement/trial",
        method: "post",
        data
      }).then(res => {
        if (res.data.code == "200") {
          ...
          listData.installmentPlanList[
            listData.installmentPlanList.length - 1
          ].dueDate = "保证金冲抵";
          this.trial = listData;
        } else {
          this.$message.error(res.data.message);
