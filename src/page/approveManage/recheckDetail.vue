  ...
  <div class="container">
    ...
    <div class="top">
      	...
      	<div class="flex-item" style="text-align: right;padding-top: 0;">
	      	...
	      	<el-button 
	      		:type="checkHeadData.status=='REVIEW_SUCCESS'?'success':
	      		checkHeadData.status=='BACK'?'info':
	      		checkHeadData.status=='REJECT'?'danger':
	      		checkHeadData.status=='REJECT'||checkHeadData.status=='RETURNED'?'warning':''" 
	      		size="mini">
      	...
      	<el-tabs v-model="activeName2" type="card"  @tab-click="handleClick">
        	<el-tab-pane label="评级打分卡" name="first">
						<div class="tab-content">
							<template>
		              <Scorecard :api-url="'recheckDetail'" :supplier-id="supplierId" :readOnly="true">
		              </Scorecard>
		          </template>
						</div>
					</el-tab-pane>
            ...
            <div class="tab-content">
            	<template v-if="verifyShow">
		              <Verify :api-url="'recheckDetail'" :supplier-id="supplierId" :readOnly="true">
		              </Verify>
		          </template>
            </div>
	        </el-tab-pane>
        	<el-tab-pane label="初审结论" name="three">
		              ...
		              <Check :api-url="'recheckDetail'" :supplier-id="supplierId" :readOnly="true" :check-data="checkData">
		              </Check>
		          </template>
						</div>
					</el-tab-pane>
        	<el-tab-pane label="复审结论" name="four">
		              ...
		              <Recheck :api-url="'recheckDetail'" :supplier-id="supplierId" :readOnly="(checkHeadData.status=='PENDING'||checkHeadData.status==='RETURNED')?false:true" :recheck-data="recheckData">
		              </Recheck>
		          </template>
						</div>
					</el-tab-pane>
         	...
         	<li class="background-gray color-black">
	      	...
	      	<li v-if="checkHeadData.status==='PENDING'||checkHeadData.status==='RETURNED'">
	            <div class="fk_btn_box">
	            	<el-button @click="doPass" v-if="!roleHasPermissions['finance_manage_register_list_audit']"  type="success" >复审通过</el-button>
	            </div>
	      	</li>
	       	<li v-if="checkHeadData.status==='PENDING'||checkHeadData.status==='RETURNED'">
	            <div class="fk_btn_box">
	              <el-button  @click="doReject " v-if="!roleHasPermissions['finance_manage_register_list_audit']" type="danger">复审拒绝</el-button>
	            </div>
	      	</li>
	       	<li v-if="checkHeadData.status==='PENDING'||checkHeadData.status==='RETURNED'">
	            <div class="fk_btn_box">
...
<script>
  import { mapGetters } from 'vuex'
	...
	export default {
    name: 'FinanceDetail',
    components: { Upload,Check,Scorecard ,Verify,Recheck},
    data () {
      	return {
...
//	  		this.verieyCheck();//尽调检查
	    ...
	    getQueryCheckHead (){//押品管理-信息登记-头部信息
	      	recheckDetail.checkHead(this.supplierId).then(res => {
      ...
      getQuerySignConclusion(){//初审授信结论-查询
      	recheckDetail.signConclusion(this.supplierId).then(res => {
          if (res.data.code === '200') {
      ...
      getQueryRecheckData(){//复审授信结论-查询
      	recheckDetail.recheckDetail(this.supplierId).then(res => {
          if (res.data.code === '200') {
...
//    	recheckDetail.scorecardQuery(this.supplierId).then(res => {
//        if (res.data.code === '200') {
...
//    },
      getSignEnterpriseStrengthList(){//企业实力和业绩单选数据
      	recheckDetail.signEnterpriseStrength(this.supplierId).then(res => {
          if (res.data.code === '200') {
...
//        	}
        ...
        this.$confirm('确认通过复审？', '操作提示', {
            ...
            recheckDetail.checkPass(this.supplierId).then(res => {
		          if (res.data.code === '200') {
        ...
        recheckDetail.checkReject(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
        ...
        recheckDetail.checkReturned(data).then(res => {
          if (res.data.code === '200') {
            this.$message.success(res.data.message)
